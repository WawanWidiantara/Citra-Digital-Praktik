...
def rotate(derajat, wrap=False):
    ...
    # r = round(abs(len(img) * cos) + abs(len(img[0]) * sin)) + 1
    # c = round(abs(len(img) * sin) + abs(len(img[0]) * cos)) + 1
    # xs = round((r - len(img))/2) - 1
    # ys = round((c - len(img[0]))/2) - 1
